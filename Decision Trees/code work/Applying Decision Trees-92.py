...
predict_train = clf.predict(train[columns])
train_auc = roc_auc_score(train['high_income'], predict_train)

predict_test = clf.predict(test[columns])
test_auc = roc_auc_score(test['high_income'], predict_test)
...
clf = DecisionTreeClassifier(random_state=1)
clf.fit(train[columns], train['high_income'])
train_pred = clf.predict(train[columns])
train_auc = roc_auc_score(train['high_income'], train_pred)
