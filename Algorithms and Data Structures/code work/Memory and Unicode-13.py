...
def clean_df(df):
    ...
    cleaned_list = [s for s in statement if ord(s) in good_characters]
    cleaned_sentence = "".join(cleaned_list)
    return cleaned_sentence

sentences_cia['cleaned_statement'] = sentences_cia.apply(clean_df, axis=1)
...
filtered_tokens = [x for x in statement_tokens if len(x) > 4]
...
def find_most_common_by_year(string):
    rows = sentences_cia[sentences_cia['year'] == string]
    combined_statement = ''.join(rows['cleaned_statement'])
    combined_list = combined_statement.split(' ')
    words = [x for x in combined_list if len(x) > 4]
    word_counts = Counter(words)
    most_common = word_counts.most_common(2)
    return most_common

common_2000 = find_most_common_by_year('2000')
common_2002 = find_most_common_by_year('2002')
common_2013 = find_most_common_by_year('2013')