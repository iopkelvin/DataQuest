...
cols_to_process = ["Age_categories", "Pclass", "Sex"]
...
predictions = pd.Series(holdout_predictions)
...
extracted_holdout = holdout["Name"].str.extract('([A-Za-z]+)\.', expand =False)
holdout['Title'] = extracted_holdout.map(titles)
...
submission = pd.DataFrame(holdout['PassengerId'], holdout_predictions, columns=['PassengerId', 'Survived'])
